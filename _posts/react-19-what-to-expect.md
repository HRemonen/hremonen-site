...
React Compiler – also known as the React Forget which optimizes [re-rendering](https://www.incluvate.com/blog/how-react-re-renders) through auto-generating the equivalent of **memo**, **useMemo** and **useCallback** calls. You might be wondering why is this important? Well, now if you must use memorization in React, it can become a hot mess quickly. You must use the **memo**, **useMemo** and **useCallback** to optimize the application. There is a mental overhead to this. You must specifically specify and update the dependencies for the hooks, which may become harder to maintain further in the development.
